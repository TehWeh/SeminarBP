...
\begin{tikzpicture}[scale=0.8,transform shape]
    ...
    \draw[-, >= stealth] (a) edge [right] node {} (x1);
...
\draw[<-, >= stealth] (b) edge [bend left, above] node[pos = 0.5] {\only<13-17>{0}} (x2);
\only<13>{\draw[<-, >= stealth, thick] (b) edge [bend left, above] node[pos = 0.5] {0} (x2);}
...
\draw[<-, >= stealth] (x5) edge [bend left, above] node {\only<15-18>{0}} (b);
\only<15>{\draw[<-, >= stealth, thick] (x5) edge [bend left, above] node {0} (b);}
