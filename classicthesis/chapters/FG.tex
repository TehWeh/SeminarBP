...
\item $V_+(a)$ is set of variables that appear unnegated in $a$ \newline $V_-(a)$ is set of $a$'s negated variables.
...
Again the circles are variable nodes, the rectangles are constraint nodes. If if $x_i$ appears negated in the clause $a$ (if $J_a^i = 1$), the edge is drawn dashed.  
